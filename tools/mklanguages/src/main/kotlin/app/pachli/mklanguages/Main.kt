...
class App : CliktCommand() {
    ...
    override fun run() {
        ...
        val languages = locales
            .sortedBy { collator.getCollationKey(casemapFold.apply(it.getDisplayName(it))) }
            .map { Language.from(it) }

        createDoNotTranslateDotXml(resourcePath, languages)
        createLocalesConfigDotXml(resourcePath, languages)
    }

    private fun createDoNotTranslateDotXml(
        resourcePath: Path,
        ...
        donottranslateXml.toFile().useLines { lines ->
            ...
            for (line in lines) {
                ...
                if (line.contains("<string-array name=\"language_entries\">")) {
                    w.println("        <!-- Autogenerated with mklanguages. -->")
                ...
                if (inLanguageEntries && line.contains("</string-array>")) {
                    ...
                    w.println("        <item>@string/system_default</item> <!-- default: System default -->")
                    languages.forEach {
    ...
    private fun createLocalesConfigDotXml(
        resourcePath: Path,
        languages: List<Language>,
    ) {
        val localesConfigXml = resourcePath / "xml" / "locales_config.xml"
        val tmpFile = createTempFile().toFile()
        val w = tmpFile.printWriter()
        w.println("""<?xml version="1.0" encoding="utf-8"?>""")
        w.println("<!-- Autogenerated with mklanguages. -->")
        w.println("""<locale-config xmlns:android="http://schemas.android.com/apk/res/android">""")
        languages.forEach {
            w.println("""    <locale android:name="${it.code}"/> <!-- ${it.displayNameEnglish} -->""")
        }
        w.println("</locale-config>")
        w.close()
        Files.move(tmpFile.toPath(), localesConfigXml, StandardCopyOption.REPLACE_EXISTING)
        log.info { "replaced ${localesConfigXml.toAbsolutePath()}" }
    }
}
