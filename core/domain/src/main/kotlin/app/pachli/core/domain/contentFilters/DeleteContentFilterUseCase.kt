...
class DeleteContentFilterUseCase @Inject constructor(
    ...
    ): Result<Unit, ContentFiltersError> {
        return externalScope.async {
            val server = accountManager.getPachliAccount(pachliAccountId)!!.server

            return@async contentFiltersRepository.deleteContentFilter(server, contentFilterId)
                .onSuccess { accountManager.deleteContentFilter(pachliAccountId, contentFilterId) }
        }.await()
    }
}
