...
import app.pachli.core.common.util.MEDIA_TEMP_PREFIX
...
class PruneCachedMediaWorker @AssistedInject constructor(
    ...
    override suspend fun doWork(): Result {
        ...
        val files = try {
            ...
            Timber.e(se, "Skipping prune, listFiles threw exception")
            return Result.failure()
        }

        if (files == null) {
            Timber.d("Skipping prune, listFiles returned null")
            return Result.success()
        }

        if (files.isEmpty()) {
            Timber.d("Skipping prune, listFiles returned empty list")
            return Result.success()
        }

        val countFiles = files.size
        var countSuccess = 0
        var countFailed = 0
        var countException = 0

        Timber.d("Starting prune of %d files", countFiles)

        files.forEach { file ->
            try {
                if (file.delete()) countSuccess++ else countFailed++
            } catch (se: SecurityException) {
        ...
        Timber.d(
            "Pruned media cache, %d/%d deleted, %d/%d failed, %d/%d exception",
            countSuccess,
            countFiles,
            countFailed,
            countFiles,
